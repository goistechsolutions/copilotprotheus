...
Static Function PQryRun(cQuery, oPSR, np, oMsg)
	...
	Begin Sequence
		...
		Else
			...
			Next
			cFCria := "db" + "Create"
			IF Select (cAliasExqr)>0;(cAliasExqr)->(DbCloseArea());EndIf
				//&(cFCria)(cDir + cAliasExqr, aEstruAux )
				DbCreate(cDir + cAliasExqr, aEstruAux/*,"SQLITE_TMP"*/)
				dbUseArea( .T., "DBFCDX", cDir + cAliasExqr, cAliasExqr, .F., .F. )
				//dbUseArea( .T., "SQLITE_TMP", cDir + cAliasExqr, cAliasExqr, .F., .F. )
...
Return  "TI" + StrTran(Time(),":","") + __cSeqTmp
