...
def clean_tenant_id(value: str | None) -> str: 
    ...
    Não permite valores vazios, reservados ou com padrão inseguro. 
    """
...
def resolve_tenant_schema(db: Any, tenant_id: str) -> str: 
    """
    Resolve o schema do tenant a partir da tabela public.tenant. 
    """
    row = db.execute( 
        text("""
            SELECT schema_name 
...
def resolve_company( 
    ...
    """
    Resolve empresa/filial ativa no schema do tenant.
    Prioriza company_id, depois company_code + branch, depois default. 
    """
...
def resolve_context( 
    ...
    """
    Resolve o contexto Protheus completo a partir do payload recebido. 
    Valida tenant, resolve schema, empresa/filial e módulo (por mod_code). 
    """
